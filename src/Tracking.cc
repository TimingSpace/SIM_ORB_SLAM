...
Tracking::Tracking(Map *pMap):
    mState(NOT_INITIALIZED),mpMap(pMap),
...
void Tracking::trackingImage(vector<string> vstrImageFilenames,Mat K,Mat DistCoef)
{
    for(int i=0;i<vstrImageFilenames.size();i++)
    {
        ...
        imshow("img",img);
        mCurrentFrame=Frame(img,0,mpORBextractor,K,DistCoef);
        ...
        else if(mState==WORKING)
        {
            ...
            bool trackSuccess=TrackPreviousFrame();
            if(trackSuccess)
            {
                TrackLocalMap();
            }
...
void Tracking::CreateInitialMap(cv::Mat &Rcw, cv::Mat &tcw)
{
    ...
    // Bundle Adjustment
    cout<<"New Map created with points  "<<mpMap->MapPointsInMap()<<endl;
...
bool Tracking::TrackPreviousFrame()
{
    ...
    cout<<"Matching number:  "<<nmatches<<endl;
